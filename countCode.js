...
function countCode(startingLetter) {
    let count = 0;
    for (let i = 0; i < data.length; i++) {
        if (data[i].charAt(0) === startingLetter) {
            count++;
        }
    }
    return count;
...
//Loeme koodid, mis algavad "i" tähega
const letterToCount = "i";
const result = countCode(letterToCount);
console.log(`Koodide arv, mis algavad "${letterToCount}" tähega: ${result}`);
